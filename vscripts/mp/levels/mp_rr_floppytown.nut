...
// Zipline Stuff
asset Metal_Beam                = $"mdl/beacon/kodai_metal_beam_02_256.rmdl"
...
{
    entity column   = CreateFloppytownModel( Metal_Beam, pos, ang, true, 20000, -1 )
    entity support  = CreateFloppytownModel( Zip_Arm, pos + <-2,-5,185>, ang, true, 20000, -1 )
...
void function PerimeterZone( vector mapPos, vector mapAng, vector areaCPos, vector areaCAng )
{
    ...
    array< int > northWallInt = [ 2, 10, 4, 10, 7, 10, 2, 1, 12, 4, 6, 1, 7 ]
    ...
    array< int > southWallInt = [ 2, 3, 6, 7, 19, 4, 3, 6 ]
    ...
    {   for ( int j=0; j<southWallInt[ 3 ] ;j++ )
        {
            CreateFloppytownModel( Square_Block, southWallPos + < -2358, 0, 1120 > + < 260.7*i, 0, 224*j >, mapAng + < 0, 0, 0 >, true, 20000, -1 )
        }
    }
    ...
    array< int > westWallInt = [ 13, 4, 11, 8, 1, 2, 1, 5 ]
    ...
    array< int > eastWallInt = [ 2, 4, 2, 5, 5, 2, 2, 14, 6, 13 ]
...
void function Build_01_10( vector mapPos, vector mapAng, vector areaCPos, vector areaCAng )
{
    ...
    {   for ( int j=0; j<build009Int[ 5 ] ;j++ )
        {
            CreateFloppytownModel( Thunderdome_Ceiling, build009Pos + < -256, 0, 2016-32 > + < 256*i, 256*j, 0 >, mapAng + < 0, 0, 0 >, true, 20000, -1 )
        }
    }
